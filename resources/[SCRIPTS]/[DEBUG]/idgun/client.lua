    ...
    while true do                                        -- Loop it infinitely.
        ...
        if infoOn then                                   -- If the info is on then...
                ...
                if IsEntityAPed(entity) then
                    ...
                    if vehicle ~= 0 then
                        if hashes[tostring(model_veh)] then
                            modelVehText = hashes
                            [tostring(model_veh)]                      -- Set the modelText local with the actual name.
                        else
            ...
            if modelVehText ~= "" then
                DrawInfos("Coordinates: " .. coordsText, "Heading: " .. headingText,
                    "Hash (PED IN VEHICLE): " .. modelText, "Hash (VEHICLE): " .. modelVehText)
            else
