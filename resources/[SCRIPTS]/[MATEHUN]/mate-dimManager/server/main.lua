...
---@param target number
---@return boolean,number|boolean,Dimension|boolean,{ gamemode: string, mode: string}|boolean
exports("GetPlayer", (function(target)
     print("GetPlayer: ", target)
     local keys = PlayerBuckets[target]
     if not keys or next(keys) == nil then
          return false, false, false, false
     end

     local dim = Dimensions[keys.gamemode][keys.mode]

     if not dim or next(dim) == nil then
          return false, false, false, false
     end

     return true, dim.bucket, dim, keys
end))
...
exports("AddPlayerToPlayer", function(playerId, toPlayer)
     if not PlayerBuckets[toPlayer] then return print(("Failed to set %s(%s) Dimension to %s(%s) dimension")) end
end)
