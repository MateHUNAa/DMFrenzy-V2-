	...
	spectate = function(self, data, cb)
		...
		if serverId == GetPlayerServerId(PlayerId()) then
			return TriggerEvent('chat:addMessage', {
				template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(0, 0, 0, 0.6); border-radius: 3px;">^1[Admin Info]: Nem nézheted meg magad.</div>'
			})
		end
...
RegisterCommand("spectateoff", function()
	if Panel.selectedPlayer then
		TriggerEvent('chat:addMessage', {
			template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(0, 0, 0, 0.6); border-radius: 3px;">^1[Admin Info]: Spectate kikapcsolva</div>'
		})
		Panel:spectateoff()
	end
end)
