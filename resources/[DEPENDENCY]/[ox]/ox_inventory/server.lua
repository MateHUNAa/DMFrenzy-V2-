...
lib.addCommand({ 'additem', 'giveitem' }, {
	...
	restricted = {"group.admincontroller", "group.ac", "group.dev", "group.developer", "group.headdeveloper", "group.manager", "group.owner"},
	...
	if item then
		...
		local metaData = {
			givvenBy = ("%s"):format(GetPlayerName(source))
		}

		if args.type then
			metaData.type = tonumber(args.type) or args.type
		end
